...
class Reservation
{
    ...
    public function creerReservation($trajetId, $passagerId, $messagePassager = null, $telephoneContact = null)
    {
        try {
            ...
            // Je récupère les données du trajet avec jointure appropriée
            ...
            // Je vérifie le crédit du passager séparément
    ...
    /**
     * Je récupère les réservations d'un utilisateur avec enrichissement des données
     */
    public function getReservationsUtilisateur($userId)
    {
        try {
            ...
            foreach ($reservations as &$reservation) {
                ...
                // Je calcule le statut d'affichage
    ...
    /**
     * J'annule une réservation avec remboursement
     */
    public function annulerReservation($reservationId, $userId, $motifAnnulation = null)
    {
        try {
            ...
            // Je vérifie que la réservation appartient à l'utilisateur et peut être annulée
            ...
            // Je rembourse les crédits
            $sql = "UPDATE utilisateurs SET credit = credit + ? WHERE id = ?";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([$reservation['credits_utilises'], $userId]);
            
            // Je remets la place disponible
            $sql = "UPDATE trajets SET places = places + 1 WHERE id = ?";
            $stmt = $this->pdo->prepare($sql);
    ...
    /**
     * Je valide un trajet effectué par un passager, libère les crédits au conducteur
     */
    public function validerTrajet($reservationId, $userId)
    {
        try {
            ...
            // Je récupère la réservation confirmée non validée
    ...
    /**
     * Je démarre toutes les réservations d'un trajet
